    ...
    public class FakeBackendHandler : HttpClientHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            async Task<HttpResponseMessage> authenticate()
            {
                ...
                var body = JsonSerializer.Deserialize<UserForAuthenticationDto>(bodyJson);
            ...
            {
                var bodyJson = await request.Content.ReadAsStringAsync(cancellationToken);
                var body = JsonSerializer.Deserialize<UserForRegistrationDto>(bodyJson);
            ...
            async Task<HttpResponseMessage> updateUser()
            {
                ...
                var bodyJson = await request.Content.ReadAsStringAsync(cancellationToken);
                var body = JsonSerializer.Deserialize<UserForEditDto>(bodyJson);
                var user = users.FirstOrDefault(x => x.Id == idFromPath());
